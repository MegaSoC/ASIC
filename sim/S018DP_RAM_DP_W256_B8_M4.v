/*
    ...
    Filename:      S018DP_RAM_DP_W256_B8_M4.v
    IP code :      S018DP
    Version:       0.1.b
    CreateDate:    Apr 18, 2021

    Verilog Model for Dual-PORT 
    SMIC 0.18um G Logic Process

    Configuration: -instname S018DP_RAM_DP_W256_B8_M4 -rows 64 -bits 8 -mux 4 
    Redundancy: Off
    Bit-Write: Off
*/
...
module S018DP_RAM_DP_W256_B8_M4 (
                         QA,
                         QB,
			  CLKA,
			  CLKB,
...
begin
  ...
  specify
    ...
    $period(posedge CLKA,2.616,CLKA_CYC_flag);
    $width(posedge CLKA,0.785,0,CLKA_H_flag);
    $width(negedge CLKA,0.785,0,CLKA_L_flag);
    
    $setuphold(posedge CLKB,posedge CENB,1.000,0.500,CENB_flag);
    $setuphold(posedge CLKB,negedge CENB,1.000,0.500,CENB_flag);
    $period(posedge CLKB,2.616,CLKB_CYC_flag);
    $width(posedge CLKB,0.785,0,CLKB_H_flag);
    $width(negedge CLKB,0.785,0,CLKB_L_flag);
